...
3D Design Studio v9
Architecture:
  - La scène JSON est injectée DANS le HTML du viewer à chaque rerun
  - st.components.v1.html est appelé avec key= basée sur un hash de la scène
    → Streamlit recrée l'iframe uniquement quand la scène change
  - La caméra est préservée via window.name (survit aux recreations iframe)
  - L'input du bus action est caché via CSS inline direct (pas de :has)
"""
...
/* Bus input hidden - direct inline style approach, no :has() */
.stApp [data-testid="stTextInput"].bus-input-wrapper{
...
def init_session():
    ...
    # Camera state survives reruns in session
    _ss("cam_sph", None)  # {theta,phi,r}
    _ss("cam_tgt", None)  # {x,y,z}
...
def save_grid_to_object(obj_df, oid, cell, extent, angle):
    ...
    if (float(o.get("grid_cell_size",10))==cell and int(o.get("grid_extent",8))==extent and int(o.get("grid_angle",0))==angle):
        return obj_df
    df2=obj_df.copy()
...
def process_viewer_action(raw, obj_df, pts_df, seg_df):
    ...
    # Restore grid state from every action
    ...
    # Preserve camera
    if "camTheta" in action:
        st.session_state["cam_sph"]={"theta":action["camTheta"],"phi":action["camPhi"],"r":action["camR"]}
        st.session_state["cam_tgt"]={"x":action["camTgtX"],"y":action["camTgtY"],"z":action["camTgtZ"]}
    ...
    if t=="grid_activate":
        ...
        save_parquet(seg_df[seg_df["segment_id"]!=int(action["id"])],SEG_KEY); st.rerun()

    elif t=="select_object":
        st.session_state["object_id"]=int(action["id"]); st.rerun()
...
def build_scene_json(project_id,obj_df,pts_df,seg_df,sel_obj,sel_pts,coinc_ids):
    ...
    scene={
        ...
        "camSph":st.session_state.get("cam_sph"),
        "camTgt":st.session_state.get("cam_tgt"),
    }
...
def scene_key(scene: dict) -> str:
    """Short hash of scene for use as iframe key — changes only when data changes."""
    # Exclude camera from hash so camera moves don't recreate iframe
    s2 = {k:v for k,v in scene.items() if k not in ("camSph","camTgt")}
    raw = json.dumps(s2, sort_keys=True, separators=(',',':'))
    return hashlib.md5(raw.encode()).hexdigest()[:12]

# ─────────────────────────────────────────────────────────────────────────────
# VIEWER HTML — built per render, injected into iframe
# ─────────────────────────────────────────────────────────────────────────────
_ANGLE_PRESETS = [0,15,30,45,60,90,120,135,150,180,225,270,315]

def build_viewer_html(scene: dict, height: int = 560) -> str:
    scene_json = json.dumps(scene, separators=(',',':'))
    presets_json = json.dumps(_ANGLE_PRESETS)
    is_plan = (scene["mode"] == "plan_editor")
...
#pt-flash{{bottom:72px;left:10px;background:rgba(10,40,10,.92);color:#3fb950;border:1px solid #3fb950;border-radius:5px;padding:5px 10px;font-size:11px;display:none;z-index:10;}}
...
<div id="wrap">
  ...
  <div id="help" class="hud">
    Clic droit+glisser → rotation<br>Molette+glisser → pan · Molette → zoom<br>
    <b>Clic point</b> → sélect + grille · <b>Clic sol</b> → grille<br>
    <b>Survol grille + clic</b> → créer point<br>
    Suppr → supprimer · Échap → fermer grille
  </div>
  <div id="ghud" class="hud"><div class="gt">⊞ grille</div><div id="gh-ang">0°</div><div class="gd" id="gh-dist">Survolez…</div></div>
...
// ═══════════════════════════════════════
// SEND ACTION → Streamlit
// Uses postMessage to parent (same-origin within Streamlit)
// AND tries direct DOM access as fallback
// ═══════════════════════════════════════
function sendAction(payload) {{
  const msg = JSON.stringify(payload);
  // Try postMessage to all ancestor frames
  try {{ window.parent.postMessage({{_3ds_action: msg}}, '*'); }} catch(e) {{}}
  // Direct DOM: look for our hidden input in parent documents
  const wins = [window.parent, window.parent?.parent].filter(Boolean);
  for (const w of wins) {{
    try {{
      // Find by data attribute we'll set on a container div
      const inp = w.document.querySelector('#_3ds_bus_input');
      if (inp) {{
        const nativeSetter = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value').set;
        nativeSetter.call(inp, msg);
...
// ── Restore camera from scene (preserved across reruns) ────────────
const cs = SCENE.camSph;
const ct = SCENE.camTgt;
let sph = cs ? {{theta:cs.theta, phi:cs.phi, r:cs.r}} : {{theta:0.6, phi:0.9, r:18}};
let tgt = ct ? new THREE.Vector3(ct.x, ct.y, ct.z) : new THREE.Vector3();
...
if (SCENE.showGrid) {{
  const g1=new THREE.GridHelper(200,200,0xe0e0e0,0xe0e0e0); g1.material.transparent=true; g1.material.opacity=0.6; bgGroup.add(g1);
...
const VGRID = {{
  ...
  angle:SCENE.gridAngle||0, cellSize:SCENE.gridCellSize||10, extent:SCENE.gridExtent||8,
...
function buildVGrid() {{
  ...
  vgridAxes();
  const N=VGRID.extent, S=VGRID.cellSize*0.01, aH=VGRID.axisH, aV=VGRID.axisV, O=VGRID.origin;
  const mL=new THREE.LineBasicMaterial({{color:0x3a7bd5,transparent:true,opacity:0.6}});
  const mO=new THREE.LineBasicMaterial({{color:0xf59e0b}});
  for(let j=-N;j<=N;j++){{
    ...
    VGRID.group.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints([s,e]),j===0?mO:mL));
  }}
  for(let i=-N;i<=N;i++){{
    ...
    VGRID.group.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints([s,e]),i===0?mO:mL));
  }}
  ...
  VGRID.hoverMesh=new THREE.Mesh(new THREE.SphereGeometry(0.09,10,8),
...
function vgridSnap(ray) {{
  ...
  return {{worldPos:snapped,iu,iv,distCm:Math.sqrt((iu*VGRID.cellSize)**2+(iv*VGRID.cellSize)**2),uCm:iu*VGRID.cellSize,vCm:iv*VGRID.cellSize}};
}}

function activateGrid(origin) {{
  VGRID.origin.copy(origin); VGRID.active=true; buildVGrid();
  document.getElementById('grid-panel').style.display='block';
  document.getElementById('ghud').style.display='block';
  sendAction({{type:'grid_activate',x:origin.x/0.01,y:origin.y/0.01,z:origin.z/0.01,angle:VGRID.angle,gridCellSize:VGRID.cellSize,gridExtent:VGRID.extent,...camPayload()}});
}}
...
function dismissGrid() {{
  ...
  VGRID.hoverPos=null;
  sendAction({{type:'grid_dismiss',...camPayload()}});
}}
...
function updateAngleDisplay(a) {{
  gpAngleVal.textContent=a+'°'; document.getElementById('gh-ang').textContent=a+'°';
  const pct=(a/359)*100;
...
function updateGridPanel() {{
  gpAngle.value=VGRID.angle; document.getElementById('gp-cell').value=VGRID.cellSize; document.getElementById('gp-ext').value=VGRID.extent;
...
PRESETS.forEach(p=>{{
  const btn=document.createElement('button'); btn.className='gp-preset'; btn.dataset.angle=p; btn.textContent=p+'°';
...
document.getElementById('gp-cell').addEventListener('input',()=>{{const v=parseFloat(document.getElementById('gp-cell').value);if(v>0){{VGRID.cellSize=Math.round(v*10)/10;buildVGrid();}}}});
document.getElementById('gp-ext').addEventListener('change',()=>{{const v=parseInt(document.getElementById('gp-ext').value);if(v>0){{VGRID.extent=v;buildVGrid();}}}});
...
// Restore grid from scene data
if (SCENE.gridOrigin) {{
  VGRID.origin.set(SCENE.gridOrigin.x*0.01, SCENE.gridOrigin.y*0.01, SCENE.gridOrigin.z*0.01);
  VGRID.angle=SCENE.gridAngle||0; VGRID.cellSize=SCENE.gridCellSize||10; VGRID.extent=SCENE.gridExtent||8;
...
// OBJECT SCENE
// ═══════════════════════════════════════
const ptGeo = new THREE.SphereGeometry(0.10, 12, 10);
const ptGeoSel = new THREE.SphereGeometry(0.13, 12, 10);
...
function buildScene() {{
  ...
  SCENE.objects.forEach(obj=>{{
    ...
    // Points — ALWAYS render in OD mode
    if (SCENE.mode==='object_designer') {{
      obj.points.forEach(p=>{{
        const isSel=(vSel.type==='point'&&vSel.id===p.id);
        const color = p.coin ? 0xff2222 : isSel ? 0xf59e0b : 0x111111;
        const emissive = isSel ? 0x332200 : 0x000000;
    ...
    // Coincident (PE)
    if (SCENE.mode==='plan_editor') {{
      obj.points.forEach(p=>{{
        if(p.coin) {{
          const m=new THREE.Mesh(new THREE.SphereGeometry(0.12,10,8),new THREE.MeshLambertMaterial({{color:0xff2222}}));
    ...
    if (SCENE.mode==='plan_editor') {{
      ...
      const proxy=new THREE.Mesh(new THREE.BoxGeometry(sz.x,sz.y,sz.z),new THREE.MeshBasicMaterial({{visible:false,side:THREE.DoubleSide}}));
      proxy.position.copy(ct); proxy.userData={{type:'object',id:obj.id}}; g.add(proxy);
    }}

    // Anchor
    if (SCENE.mode==='plan_editor'&&obj.sel) {{
...
const pickRay=new THREE.Raycaster(); pickRay.params.Line={{threshold:0.08}};

function camPayload() {{
  return {{camTheta:sph.theta,camPhi:sph.phi,camR:sph.r,camTgtX:tgt.x,camTgtY:tgt.y,camTgtZ:tgt.z}};
}}
...
function onMouseMove(ev) {{
  ...
  if(VGRID.active&&VGRID.hoverMesh) {{
    const sn=vgridSnap(pickRay);
    if(sn){{
      VGRID.hoverMesh.position.copy(sn.worldPos); VGRID.hoverMesh.visible=true; VGRID.hoverPos=sn;
      document.getElementById('gh-dist').textContent=`${{sn.distCm.toFixed(1)}} cm  H:${{sn.uCm.toFixed(1)}} V:${{sn.vCm.toFixed(1)}}`;
    }}else{{VGRID.hoverMesh.visible=false;VGRID.hoverPos=null;document.getElementById('gh-dist').textContent='Survolez…';}}
  }}
}}

// ═══════════════════════════════════════
// CLICK — objects first, then grid, then ground
// ═══════════════════════════════════════
let flashTimer=null;
function showFlash(msg){{
  const el=document.getElementById('pt-flash'); el.textContent='✓ '+msg; el.style.display='block';
  if(flashTimer)clearTimeout(flashTimer); flashTimer=setTimeout(()=>{{el.style.display='none';}},2500);
}}
...
cv.addEventListener('click', ev=>{{
  ...
  // ── PRIORITY 1: pick existing points/objects ──────────────────
  const tgts=[];
    ...
    if(SCENE.mode==='plan_editor'){{
      ...
      if(ud.type==='point'){{
        ...
        if(obj){{
          ...
          if(pt) activateGrid(new THREE.Vector3((obj.pos.x+pt.x)*0.01,(obj.pos.y+pt.y)*0.01,(obj.pos.z+pt.z)*0.01));
        }}
      }}
    }}
    return;
  ...
  if(VGRID.active&&VGRID.hoverPos){{
    ...
    sendAction({{
      type: SCENE.mode==='object_designer'?'grid_click_od':'grid_click_pe',
    ...
    if(SCENE.mode==='object_designer') showFlash(`(${{(p.x/0.01).toFixed(1)}}, ${{(p.y/0.01).toFixed(1)}}, ${{(p.z/0.01).toFixed(1)}}) cm`);
    return;
  }}
  ...
  if(pickRay.ray.intersectPlane(gndPl,gHit)){{
    activateGrid(gHit);
    document.getElementById('status').textContent='Grille verticale — survolez puis cliquez';
  }}
}});
...
window.addEventListener('keydown',ev=>{{
  ...
  if((ev.key==='Delete'||ev.key==='Backspace')&&SCENE.mode==='object_designer'){{
    if(vSel.type==='point'){{
      sendAction({{type:'delete_point',id:vSel.id,
        gridOriginX:VGRID.origin.x/0.01,gridOriginY:VGRID.origin.y/0.01,gridOriginZ:VGRID.origin.z/0.01,
        gridAngle:VGRID.angle,gridCellSize:VGRID.cellSize,gridExtent:VGRID.extent,...camPayload()}});
...
// RESIZE + RENDER LOOP
// ═══════════════════════════════════════
new ResizeObserver(()=>{{renderer.setSize(W(),H());camera.aspect=W()/H();camera.updateProjectionMatrix();}}).observe(wrap);
...
# ─────────────────────────────────────────────────────────────────────────────
# ACTION BUS — hidden input using direct DOM id (no :has CSS needed)
# ─────────────────────────────────────────────────────────────────────────────
def render_action_bus() -> str:
    """Render a hidden input that the JS viewer writes to via its id."""
    # We use st.text_input but wrap it in a div with display:none
    st.markdown('<div id="_3ds_bus_wrap" style="position:fixed;top:-9999px;left:-9999px;width:1px;height:1px;overflow:hidden;opacity:0;pointer-events:none">',
                unsafe_allow_html=True)
    val = st.text_input("bus", key="_viewer_msg", label_visibility="collapsed")
    # Inject a script that moves the actual input to have id="_3ds_bus_input"
    st.markdown("""<script>
(function(){
  const wrap = document.getElementById('_3ds_bus_wrap');
  if(wrap){
    const inp = wrap.querySelector('input');
    if(inp) inp.id = '_3ds_bus_input';
  }
})();
...
# ─────────────────────────────────────────────────────────────────────────────
# PANELS (plan editor + object designer)
# ─────────────────────────────────────────────────────────────────────────────
def _oidx(df, oid):
    m=df.index[df["object_id"]==oid]; return m[0] if len(m) else None
...
def panel_plan_editor(obj_df, pts_df, seg_df, sel_oid, coinc_ids):
    ...
    with tabs[3]:
        ...
        o_a=pts_df[pts_df["object_id"]==sel_oid] if not pts_df.empty else pd.DataFrame()
        if not o_a.empty:
            pm={f"#{int(r['point_id'])} ({float(r['x']):.1f},{float(r['y']):.1f},{float(r['z']):.1f})":(float(r["x"]),float(r["y"]),float(r["z"])) for _,r in o_a.iterrows()}
            pm["Origine (0,0,0)"]=(0.,0.,0.)
            ch=st.selectbox("Ancre",list(pm.keys()),key="anch_pick")
            if st.button("Définir",key="set_anch"):
                p2,py2,pz2=pm[ch]; idx=_oidx(obj_df,sel_oid)
                if idx is not None:
                    df2=obj_df.copy(); df2.at[idx,"anchor_x"]=p2;df2.at[idx,"anchor_y"]=py2;df2.at[idx,"anchor_z"]=pz2
    ...
    with tabs[4]:
        ...
        if st.button("Appliquer",key="abs_pos"):
            ...
            if idx is not None:
                ...
                save_parquet(df2,OBJ_KEY); st.rerun()
    with tabs[5]:
        o2=pts_df[pts_df["object_id"]==sel_oid] if not pts_df.empty else pd.DataFrame()
        if o2.empty: st.info("Ajoutez des points d'abord.")
        else:
            pm3={f"#{int(r['point_id'])} ({float(r['x']):.1f},{float(r['y']):.1f},{float(r['z']):.1f})":(float(r["x"]),float(r["y"]),float(r["z"])) for _,r in o2.iterrows()}
...
def panel_object_designer(obj_df, pts_df, seg_df, sel_oid):
    ...
    if sel_oid is None:
        st.markdown('<div class="info-box">👆 Sélectionnez un objet.<br><br>'
                    '🖱 <b>Clic sol</b> → grille verticale · <b>Clic point</b> → sélect + grille au point<br>'
...
def main():
    ...
    viewer_msg = render_action_bus()
    if viewer_msg and viewer_msg not in ("", "{}"):
        try:
            action = json.loads(viewer_msg)
            if action.get("type"):
    ...
    key=scene_key(scene)  # changes only when data changes
    ...
    # ── VIEWER — rebuilt with fresh scene, key prevents camera reset ──
    viewer_html = build_viewer_html(scene, height=560)
    st.markdown('<div class="viewer-wrap">', unsafe_allow_html=True)
    components.html(viewer_html, height=564, scrolling=False)
    st.markdown("</div>", unsafe_allow_html=True)
